...
class TagAndProbeFitter:
    ...
    def set_fit_var(self, v='x', vMin=60, vMax=140, unit='GeV', label='m(#mu#mu)'):
        self._fitVar = v
        self._fitVarMin = vMin
        self._fitVarMax = vMax
        self._w.factory('{}[{}, {}]'.format(v,vMin,vMax))
        if unit: self._w.var(v).setUnit(unit)
        if label:
    ...
    def set_histograms(self, hPass, hFail, peak=90):
        ...
        dhPass = ROOT.RooDataHist('hPass', 'hPass', ROOT.RooArgList(self._w.var(self._fitVar)), hPass)
        dhFail = ROOT.RooDataHist('hFail', 'hFail', ROOT.RooArgList(self._w.var(self._fitVar)), hFail)
    ...
    def set_gen_shapes(self, hPass, hFail, peak=90):
        ...
        dhPass = ROOT.RooDataHist('hGenPass', 'hGenPass', ROOT.RooArgList(self._w.var(self._fitVar)), hPass)
        dhFail = ROOT.RooDataHist('hGenFail', 'hGenFail', ROOT.RooArgList(self._w.var(self._fitVar)), hFail)
        self.wsimport(dhPass)
        self.wsimport(dhFail)
    ...
    def set_workspace(self, lines, template=True):
        ...
        # common lines, consider moving it outside in case we dont want to build the convolution when we fit PDF only
        if template:
            self._w.factory("HistPdf::sigPhysPass({},hGenPass)".format(self._fitVar))
            self._w.factory("HistPdf::sigPhysFail({},hGenFail)".format(self._fitVar))
            self._w.factory("FCONV::sigPass({}, sigPhysPass , sigResPass)".format(self._fitVar))
            self._w.factory("FCONV::sigFail({}, sigPhysFail , sigResFail)".format(self._fitVar))
    ...
    def fit(self, outFName, mcTruth=False, template=True):
        ...
        if template:
            self._w.var('sigmaF').setVal(self._w.var('sigmaP').getVal())
            self._w.var('sigmaF').setRange(0.8*self._w.var('sigmaP').getVal(), 3.0*self._w.var('sigmaP').getVal())
        ...
        # plot
        pFrame = self._w.var(self._fitVar).frame(self._fitRangeMin,self._fitRangeMax)
        ...
        fFrame = self._w.var(self._fitVar).frame(self._fitRangeMin,self._fitRangeMax)
        ...
        nTot = nP+nF
        eff = nP / (nP+nF)
        e_eff = 1./(nTot*nTot) * ( nP*nP* e_nF*e_nF + nF*nF * e_nP*e_nP )**0.5
        ...
        for p in listParFinalP:
            ...
            text.AddText("   - {} \t= {:.3f} #pm {:.3f}".format(pName, pVar.getVal(), pVar.getError()))
