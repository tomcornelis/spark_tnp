...
def run_conversion(spark,particle,resonance,era,subEra,shift='Nominal',**kwargs):
    ...
    job = f'{particle}_{resonance}_{era}_{subEra}'
    ...
    for num_den_binVars in yields:
        ...
        if _denominator and den not in _denominator: continue
...
def run_all(spark,particle,resonance,era,shift='Nominal',**kwargs):
    for subEra in subEras.get(resonance,{}).get(era,[]):
        run_conversion(spark,particle,resonance,era,subEra,shift,**kwargs)
...
def run_spark(particle,resonance,era,**kwargs):
    ...
    for shiftType in shiftTypes:
        if _shiftType and shiftType not in _shiftType: continue
